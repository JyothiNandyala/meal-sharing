      ...
      if (availableReservations) {
        ...
        let mealsWithAvailableReservations = await knex("meals")
          .select([
            ...
            knex.raw(
              'meals.max_reservations - sum(reservation.number_of_guests) as "available_space"'
            ),
          ])
          ...
          .having(
            knex.raw(
              "meals.max_reservations > sum(reservation.number_of_guests)"
            )
          );

        const reservationMealsWithId = knex("reservation").select("meal_id");
        const mealsWithNotReserved = await knex("meals")
          .select("id", "title", "max_reservations")
          .where("id", "not in", reservationMealsWithId);
        return response.json(
          mealsWithNotReserved.concat(mealsWithAvailableReservations)
        );
      } else {
...
router.put("/:id", async (request, response) => {
  try {
    // knex syntax for selecting things. Look up the documentation for knex for further info
    const updateTheMeal = await knex("meals")
